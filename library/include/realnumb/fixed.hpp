...
public:
    ...
    static constexpr auto total_bits = static_cast<unsigned int>(sizeof(BaseType) * 8u);
    ...
    static constexpr auto GetLowest() noexcept -> fixed
    {
        // lowest reserved for NaN
    ...
    template <typename T>
    static constexpr auto GetFromFloat(T val) noexcept
    ...
    template <typename T>
    static constexpr auto GetFromSignedInt(T val) noexcept
        -> std::enable_if_t<std::is_integral_v<T> && std::is_signed_v<T>, value_type>
    {
        return (val > (GetMax().m_value / scale_factor))? GetInfinity().m_value:
            (val < (GetLowest().m_value / scale_factor))? GetNegativeInfinity().m_value:
            static_cast<value_type>(val * scale_factor);
    }

    /// @brief Gets the value from an unsigned integral value.
    template <typename T>
    static constexpr auto GetFromUnsignedInt(T val) noexcept
        -> std::enable_if_t<std::is_integral_v<T> && !std::is_signed_v<T>, value_type>
    {
        const auto max = static_cast<unsigned_wider_type>(GetMax().m_value / scale_factor);
        return (val > max)? GetInfinity().m_value: static_cast<value_type>(val) * scale_factor;
    }
    ...
    template <typename T>
    constexpr auto ConvertTo() const noexcept -> T
    {
...
{
    return lhs.Compare(rhs) == fixed<BT, FB>::ordering::equivalent;
}
...
constexpr bool operator< (fixed<BT, FB> lhs, fixed<BT, FB> rhs) noexcept
{
    return lhs.Compare(rhs) == fixed<BT, FB>::ordering::less;
}
...
constexpr bool operator> (fixed<BT, FB> lhs, fixed<BT, FB> rhs) noexcept
{
    return lhs.Compare(rhs) == fixed<BT, FB>::ordering::greater;
}
...
constexpr bool operator<= (fixed<BT, FB> lhs, fixed<BT, FB> rhs) noexcept
{
    ...
    return result == fixed<BT, FB>::ordering::less ||
           result == fixed<BT, FB>::ordering::equivalent;
}
...
{
    const auto result = lhs.Compare(rhs);
    return result == fixed<BT, FB>::ordering::greater || result == fixed<BT, FB>::ordering::equivalent;
}
...
{
    return lhs.Compare(rhs) == fixed32::ordering::equivalent;
}
...
{
    const auto result = lhs.Compare(rhs);
    return (result == fixed32::ordering::less) || (result == fixed32::ordering::equivalent);
}
...
{
    const auto result = lhs.Compare(rhs);
    return (result == fixed32::ordering::greater) || (result == fixed32::ordering::equivalent);
}
...
{
    const auto result = lhs.Compare(rhs);
    return result == fixed32::ordering::less;
}
...
{
    const auto result = lhs.Compare(rhs);
    return result == fixed32::ordering::greater;
}
...
{
    return lhs.Compare(rhs) == fixed64::ordering::equivalent;
}
...
{
    const auto result = lhs.Compare(rhs);
    return (result == fixed64::ordering::less) || (result == fixed64::ordering::equivalent);
}

constexpr bool operator >= (fixed64 lhs, fixed64 rhs) noexcept
{
    const auto result = lhs.Compare(rhs);
    return (result == fixed64::ordering::greater) || (result == fixed64::ordering::equivalent);
}

constexpr bool operator < (fixed64 lhs, fixed64 rhs) noexcept
{
    const auto result = lhs.Compare(rhs);
    return result == fixed64::ordering::less;
}

constexpr bool operator > (fixed64 lhs, fixed64 rhs) noexcept
{
    const auto result = lhs.Compare(rhs);
    return result == fixed64::ordering::greater;
}
